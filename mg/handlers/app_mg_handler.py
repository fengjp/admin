...
class LogHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        methodKey = self.get_argument('methodKey', default=None, strip=True)
        ...
        start_time_tuple = time.strptime(str(start_date) + ' 00:00:00', '%Y-%m-%d %H:%M:%S')
        end_time_tuple = time.strptime(str(end_date) + ' 23:59:59', '%Y-%m-%d %H:%M:%S')
        log_list = []

        with DBContext('r') as session:
            if (key and value) or (methodKey and methodKey != '全部'):
                conditions = []
                if methodKey and methodKey != '全部':
                    conditions.append(or_(OperationRecord.method == methodKey))
                if key and value:
                    conditions.append(or_(OperationRecord.username.like('%{}%'.format(value)),
                                          OperationRecord.nickname.like('%{}%'.format(value)),
                                          OperationRecord.login_ip.like('%{}%'.format(value)),
                                          OperationRecord.uri.like('%{}%'.format(value)),
                                          OperationRecord.data.like('%{}%'.format(value)),
                                          ))
                conditions.append(OperationRecord.ctime > start_time_tuple)
                conditions.append(OperationRecord.ctime < end_time_tuple)
                count = session.query(OperationRecord).filter(*conditions).count()
                log_info = session.query(OperationRecord).filter(*conditions).order_by(-OperationRecord.ctime).limit(
                    int(limit))
            else:
...
class ResetPasswordHandler(BaseHandler):
    def put(self, *args, **kwargs):
        ...
        return self.write(dict(code=0, msg='重置密码成功，密码:123456'))
